  ...
  <section v-if="posts.length > 0" class="post-list">
    ...
    >
      ...
      <div class="row">
        ...
        <button class="action-btn" @click.stop="$emit('openPostDetail', post.id)">
          <span>💬</span> {{ post.commentCount ?? 0 }}
        </button>
      </div>
    </article>
  </section>
  <div v-else-if="loading" class="page-lite muted">帖子加载中...</div>
  <div v-else class="page-lite muted">暂无帖子</div>
</template>

<script setup lang="ts">
import type { CommunityPostItem } from '../../buyer/stores/useBuyerCommunityStore'
